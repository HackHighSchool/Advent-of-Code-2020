...
class Node:
    ...
    #Process the first child node (1 + the value)
        if self.val+1 == seq[len(seq)-1]: #If the sequence succesfully reaches the end of the array, stop building this branch (Works like DNA Stop Codon)
            self.first = "stop"
    ...
    #Process the second child node (2 + the value)
        if self.val+2 == seq[len(seq)-1]: #If the sequence succesfully reaches the end of the array, stop building this branch
            self.second = "stop"
            self.increment_sequence_count() #Since the sequence is successful, call the increment_sequence_count() method
    ...
    #Process the third child node (3 + the value)
        if self.val+3 == seq[len(seq)-1]: #If the sequence succesfully reaches the end of the array, stop building this branch
            self.third = "stop"
            self.increment_sequence_count() #Since the sequence is successful, call the increment_sequence_count() method
